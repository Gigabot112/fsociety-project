    ...
    return InlineKeyboardMarkup([
        ...
        [
            InlineKeyboardButton(
                "✅ Я подтвердил оплату",
                callback_data=(
                    f"USER_PAYMENT_CONFIRMED:{price}"
                )
            )
        ],
    ])
...
async def user_payment_confirmed(
    ...
    if report["user_id"] != update.effective_user.id:
    ...
    if report["status"] == (
        ...
        await query.message.reply_text(
            "⏳ Вы уже подтвердили оплату.\n\n"
            "Администратор проверяет получение подарка."
        )
    ...
    await context.bot.send_message(
        ...
        text=(
            ...
            f"Chat ID: {report['user_id']}\n\n"
            "Пользователь утверждает, что отправил "
            f"{price} ⭐ администратору.\n\n"
...
async def admin_cancel(
    ...
    await query.edit_message_text(
        ...
        f"🆔 Заявка: {report_id}\n"
        f"Пользователь: {report['username']}\n\n"
        "Напишите причину отмены следующим сообщением.\n\n"
        "Например:\n"
        "«Эта заявка была подана в шутку и "
        "не предназначена для реального рассмотрения.»\n\n"
        "Ваш следующий текст будет отправлен пользователю."
    )
...
async def message_handler(
    ...
    # =====================================================
    # АДМИН — ПРИЧИНА ОТМЕНЫ
    # =====================================================

    if (
        message.from_user
        and message.from_user.id == ADMIN_CHAT_ID
        and context.user_data.get("cancel_report")
    ):
        ...
        if not message.text:
            ...
            await message.reply_text(
                "Пожалуйста, напишите причину отмены текстом."
            )
